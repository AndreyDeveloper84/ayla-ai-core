...
- **Только слово `inferred` считалось выводом — остальной словарь
  происхождения проезжал в блок как прямая речь клиента** (P0-3 follow-up).
  `build_memory_block` определял происхождение сравнением на точное
  совпадение с `SOURCE_INFERRED`. Словарь при этом ведут три репозитория, и
  они успели разойтись:

  | значение | кто ставит | как читалось ДО |
  |---|---|---|
  | `explicit` | мобильный PATCH, внутренний PATCH бота (умолчание), фолбэк `_green_data_sources` | цитата ✔ |
  | `inferred` | ночной Celery (`users.infer_user_patterns`) | вывод ✔ |
  | `conversational` | бот, `memory_ask` — ответ на прямой вопрос | **цитата ✘** |
  | `behavioral` | принимает `_SOURCE_CHOICES`, писателя пока нет | **цитата ✘** |
  | `transactional` | принимает `_SOURCE_CHOICES`, писателя пока нет | **цитата ✘** |
  | `erased` | стирание поля (`personal_context_erasure`) | **цитата ✘** |
  | незнакомое | — | **цитата ✘** |

  **Живо ли это сегодня — нет, и это существенно.** Единственный вызывающий
  `build_memory_block` в контуре — бот, а он нормализует происхождение до
  `SOURCE_STATED`/`SOURCE_INFERRED` ещё до вызова, так что до библиотеки
  сырое `behavioral` пока не доезжает. На пилоте (`users_userpersonalcontext.
  data_sources`, 5 строк) лежит ровно одно значение — `favorite_masters:
  "inferred"`. Дефект — заряженное ружьё, а не выстрел: он срабатывает у
  первого же вызывающего, который передаст карту бэкенда как есть, а
  документация аргумента ровно к этому и приглашала. В `djangoproject` PR
  #276 такой вызывающий уже написан.

  Ошибка несимметрична: пометить догадкой то, что человек сказал сам, —
  неприятно, но безобидно; выдать домысел системы за слова клиента — то,
  чего продукт избегает намеренно (OD_C04 §1: WHY — только пересказ
  сказанного). Поэтому правило перевёрнуто и совпало с тем, которое бот
  применял у себя (`ai-bot-platform/apps/orchestrator/memory_block.py`):
  **цитатой считается только объявленное «сказал сам», всё прочее — вывод,
  включая незнакомое библиотеке значение.** Безопасная сторона стала
  умолчанием, и появление в контуре нового вида вывода правки библиотеки не
  требует.

  Словарь сведён в одно место — сюда, — чтобы три репозитория не
  доучивали его независимо. `STATED_SOURCES` **закрыт и load-bearing**:
  именно он решает исход, и каждое добавленное в него значение —
  осознанное разрешение выдать факт за прямую речь. `DERIVED_SOURCES`
  **открыт и информативен**: на исход не влияет (правило — дополнение к
  `STATED_SOURCES`), но даёт контуру один перечень, на который можно
  сослаться, и гоняет тесты по всему словарю.

  `SOURCE_EXPLICIT` — имя бэкенда для того же, что библиотека звала
  `SOURCE_STATED`. Без него было бы хуже, а не лучше: внутренний GET отдаёт
  **все двенадцать** зелёных полей, непомеченные — со значением `explicit`,
  так что вся анкета уехала бы в секцию догадок целиком.

  Граница совместимости стала точнее: **отсутствие ключа — не значение**,
  а «происхождение не сообщили», и рендерится как раньше. Схлопни отсутствие
  с присланным незнакомым значением — и бэкенд, ещё не отдающий провенанс,
  превратил бы всю анкету в догадку при первом же деплое.

  Доказательства: тест по всему словарю (пять выводов, а не один
  `behavioral`), положительная стража на обе формы «сказал сам» (без неё
  «всё считается выводом» прошло бы и на полностью сломанной правке),
  незнакомое значение, отсутствие ключа против незнакомого значения, сырая
  карта `data_sources` в форме внутреннего GET, склеенный из двух ключей
  бюджет по всему словарю, непересечение сторон перечня. Мутация «вернуть
  сравнение на точное совпадение» роняет 8 тестов; мутация «считать выводом
  всё присланное» — 9, из них две положительные стражи.
...
Почти чисто аддитивно: одиннадцать новых экспортируемых констант, один
keyword-only аргумент с умолчанием `None`. Вызывающие, которые не передают
`sources`, получают **байт-в-байт** прежний вывод — это по-прежнему
утверждает `tests/test_memory.py::test_without_sources_output_is_byte_identical`.
`tests/test_public_api_surface.py::EXPECTED_PUBLIC_API` растёт на одиннадцать
символов (`INFERRED_MARK`, `MEMORY_INFERRED_HEADER`, `SOURCE_STATED`,
`SOURCE_INFERRED`, `SOURCE_EXPLICIT`, `SOURCE_BEHAVIORAL`,
`SOURCE_CONVERSATIONAL`, `SOURCE_TRANSACTIONAL`, `SOURCE_ERASED`,
`STATED_SOURCES`, `DERIVED_SOURCES`).

**Одно поведение изменилось намеренно** — и это тот самый дефект:
`sources={"field": <не из STATED_SOURCES>}` больше НЕ рендерится «как
раньше», а помечается выводом. Кого это касается:

* **`ai-bot-platform` (пин `ee6425a`)** — не касается. Бот нормализует
  происхождение до `SOURCE_STATED`/`SOURCE_INFERRED` ещё до вызова
  (`memory_block.py:132-137`), оба значения на прежней стороне. Вывод не
  меняется ни на один байт. Нормализация теперь избыточна и может быть
  снята, но это упрощение, а не требование.
* **`djangoproject` (пин `f773e7d`, открыт PR #276 на `ee6425a`)** — прямо
  выигрывает. Его тест `TestBotPromptRendererPin` уже передаёт **сырую**
  карту `payload["data_sources"]` в `build_memory_block` без нормализации;
  на `ee6425a` это работает только потому, что в тесте стоит `inferred`.
  С этой правкой сырую карту можно передавать всегда.
* **Любой вызывающий, который передавал произвольные значения, ожидая, что
  они молча сойдут за цитату,** — сломается, и намеренно.
