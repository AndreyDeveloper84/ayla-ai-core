...
> **Context-only copy.** This file is a **mirror** of the canonical ADR maintained in `ai-bot-platform/docs/adr/ADR-0009-ayla-split-domain-architecture.md`. ayla-ai-core itself is **unchanged** by this architecture decision — see the ayla-ai-core-specific note below.
>
> ### ayla-ai-core role under ADR-0009
>
> Per ADR-0009 §Repo roles: `ayla-ai-core` is a **«Pure Python AI library. Unchanged. v0.8.1 → v1.0 freeze. Pinned via `git+ssh@vX.Y.Z` in both consumers.»**
>
> Concretely:
>
> - **No code changes** flow from this ADR into ayla-ai-core. The library continues to ship `AIConcierge`, `BrandVoiceConfig`, provider adapters, tool schemas, anti-hallucination helpers, and replay primitives.
> - Both consumers (`ai-bot-platform` and Ayla djangoproject) pin ayla-ai-core via `git+ssh@vX.Y.Z` per [`RELEASING.md`](../RELEASING.md). SHA-pinning policy unchanged.
> - The «v0.8.1 → v1.0 freeze» means: hold the public API surface stable through Phase 0. Bug fixes + security patches per [`LTS_POLICY.md`](../LTS_POLICY.md) still ship. Breaking API changes wait for v1.0 (Phase 1 timing).
>
> This file exists so consumers reading ayla-ai-core's source can find the architecture context that explains *why* the library is locked. If the ai-bot-platform copy diverges from this one, the ai-bot-platform copy is authoritative.
