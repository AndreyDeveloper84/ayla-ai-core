"""Tests для context.py — SpecialistCandidate / SpecialistContext / render_summary_text."""
...
def test_render_summary_caps_at_5_services_with_overflow_note() -> None:
    ...
    candidates = [SpecialistCandidate(id=1, name="X", specialization="", services=services)]
    out = render_summary_text(candidates)
    # First 5 — отображены
    for i in range(1, 6):
...
def test_render_summary_no_specialization_no_parens() -> None:
    candidates = [SpecialistCandidate(id=1, name="X", specialization="", services=[])]
    out = render_summary_text(candidates)
...
def test_build_context_collects_all_service_ids_across_masters() -> None:
    candidates = [
        SpecialistCandidate(id=1, name="A", specialization="", services=[(10, "s1"), (11, "s2")]),
        SpecialistCandidate(id=2, name="B", specialization="", services=[(11, "s2"), (12, "s3")]),
    ]
...
def test_build_context_empty_returns_empty_frozensets() -> None:
    ctx = build_specialist_context_from_candidates([], tenant_id="test-tenant")
...
# ─── SpecialistContext immutability (frozen dataclass) ────────────────────────
...
def test_specialist_context_classes_importable() -> None:
    """v0.8.0 (Arch-4 / DRF-688): MasterContext / MasterCandidate aliases
    removed. SpecialistContext / SpecialistCandidate remain the only
    canonical names."""
    from ayla_ai_core.context import SpecialistCandidate, SpecialistContext

    assert SpecialistContext is not None
    assert SpecialistCandidate is not None
...
def test_builder_populates_by_id_for_o1_handler_lookups() -> None:
    ...
    candidates = [
        SpecialistCandidate(id=1, name="Anna", specialization="massage", services=[(10, "back")]),
        SpecialistCandidate(id=2, name="Boris", specialization="manicure", services=[(11, "nails")]),
    ]
